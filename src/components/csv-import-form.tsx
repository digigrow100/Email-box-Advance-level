...
export function CsvImportForm() {
  ...
  return <form onSubmit={submit} className="card p-5"><h2 className="font-bold">Paste CSV</h2><p className="mt-1 text-sm text-slate-500">Required column: email. Optional: first_name, last_name, company.</p><textarea value={csv} onChange={(e: any) => setCsv(e.target.value)} className="mt-4 min-h-72 w-full rounded-xl border border-slate-200 p-3 font-mono text-sm" /><button className="mt-4 rounded-xl bg-indigo-600 px-4 py-2.5 text-sm font-semibold text-white">Import contacts</button>{message && <p className="mt-3 text-sm text-slate-600">{message}</p>}</form>;
}
