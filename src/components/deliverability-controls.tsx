...
type Seed = { id: string; mailbox_id: string; label: string; provider_hint: string; mailboxes?: { email?: string } | null };
type PlacementResult = { id: string; recipient_email: string; placement: string; matched_folder?: string | null; details?: { authentication?: { spf?: string; dkim?: string; dmarc?: string } | null } | null; seed_inboxes?: { label?: string; provider_hint?: string } | null };
type PlacementTest = { id: string; marker: string; subject: string; status: string; created_at: string; mailboxes?: { email?: string } | null; placement_results?: PlacementResult[] };
  ...
  return <div className="space-y-6">
    <div className="grid gap-6 xl:grid-cols-2">
      ...
      <form onSubmit={analyzeContent} className="card p-5">
        ...
        {risk?.risk && <div className="mt-4 rounded-xl bg-slate-50 p-4"><div className="flex items-center justify-between"><strong className="text-sm capitalize">{risk.risk} risk</strong><span className="text-sm font-bold">{risk.score}/100 risk points</span></div><ul className="mt-3 space-y-1 text-xs leading-5 text-slate-600">{risk.findings?.map((f:any,i:number)=><li key={i}>• {f.message}</li>)}</ul></div>}
